...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (1) initial hidden state from features
        ...
        # (4) vocab scores at each timestep
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        prev_h = features.mm(W_proj) + b_proj
        prev_c = torch.zeros_like(prev_h) # Initialize cell state to zeros (for LSTM)
        
        # Initial word is <START>
        current_word_idx = torch.full((N, 1), self._start, dtype=torch.long, device=features.device) # (N, 1)

        for t in range(max_length):
            # 1. Embed current word
            # W_embed: (V, W)
            # current_word_idx: (N, 1) -> x: (N, 1, W) -> squeeze -> (N, W)
            # word_embedding_forward expects (N, T). Here we can pass (N, 1) or just do manual lookup
            # Using logic consistent with training:
            x = word_embedding_forward(current_word_idx, W_embed) # (N, 1, W)
            x = x.squeeze(1) # (N, W)
            ...
            # 3. Affine to vocab
            # scores: (N, V) assuming temporal_affine can handle T=1 or just manual calculation.
            # temporal_affine_forward expects (N, T, H) -> (N, T, V).
            # Let's just use affine logic directly since it's one timestep.
            # scores = prev_h.mm(W_vocab) + b_vocab
            # The params W_vocab is (H, V)
            scores = prev_h.mm(W_vocab) + b_vocab
            
            # 4. Select max score
            _, predicted_idx = torch.max(scores, dim=1) # (N,)
            
            captions[:, t] = predicted_idx
            current_word_idx = predicted_idx.unsqueeze(1) # (N, 1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
