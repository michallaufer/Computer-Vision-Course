...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Keep tensors on the same device as inputs (important for GPU training)
        ...
        # 1) Text: token embedding + positional encoding
        # captions: (N, T) -> (N, T, W)
        tgt = self.embedding(captions)
        tgt = self.positional_encoding(tgt)

        # 2) Image: project visual features to word embedding dim
        # features: (N, D) -> (N, W) -> (N, 1, W)
        memory = self.visual_projection(features).unsqueeze(1)

        # 3) Causal (future) mask for the decoder self-attention
        # We allow attending to current and past positions only.
        # Shape: (T, T) with 1s on and below diagonal, 0s above diagonal.
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device))

        # 4) Decode and project to vocabulary scores
        out = self.transformer(tgt=tgt, memory=memory, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1) Patchify + linear projection: (N, C, H, W) -> (N, P, D)
        patches = self.patch_embed(x)

        # 2) Positional encoding: (N, P, D) -> (N, P, D)
        patches = self.positional_encoding(patches)

        # 3) Transformer encoder over patch tokens
        enc = self.transformer(patches)

        # 4) Pool over patches (no CLS token in this simplified ViT)
        pooled = torch.mean(enc, dim=1)  # (N, D)
