...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        sqrt_recip_alphas_cumprod = extract(torch.sqrt(1.0 / self.alphas_cumprod), t, x_t.shape)
        sqrt_recipm1_alphas_cumprod = extract(torch.sqrt(1.0 / self.alphas_cumprod - 1), t, x_t.shape)
        x_start = sqrt_recip_alphas_cumprod * x_t - sqrt_recipm1_alphas_cumprod * noise
        ####################################################################
        return x_start
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # no noise when t == 0
        mask = (t > 0).float()
        mask = mask.view(-1, *([1]*(len(x_t.shape)-1)))
        
        x_tm1 = posterior_mean + mask * posterior_std * noise
        ##################################################################
