...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    image_input = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
    ...
    similarity = get_similarity_no_loop(text_features, image_features) # (N_texts, N_images)
    
    # Argmax over texts (dim 0) for each image
    # Shape of probability is (N_texts, N_images). We want best text for each image.
    best_text_indices = similarity.argmax(dim=0) # (N_images,)
    
    pred_classes = [class_texts[idx] for idx in best_text_indices]
